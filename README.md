...
凭证配置保存在当前 macOS 用户的本地应用偏好设置中，秘密材料保存在 macOS Keychain，不会写入应用包，也不会提交到仓库。设置页会按供应商分组管理多个独立凭证；每个凭证可以单独启用、停用、刷新和删除。

菜单栏指标最多选择 4 个凭证。具有明确额度上限的供应商显示真实用量进度；只有余额接口的供应商（例如 DeepSeek）显示余额健康状态，不显示伪造的用量百分比。

升级旧版本时，原有 Routin `plan-` Key 会从旧本地存储迁移到 Keychain；迁移失败时会保留旧值并继续显示可恢复错误。
...
卸载应用不会自动删除本地配置或 Keychain 项。如需彻底清除数据，请先在设置中删除所有凭证，再卸载应用。
