...
Routin 每日签到使用内置网页承载 Routin 官方登录和签到流程。应用不会保存 Routin 的账号密码、验证码、Token 或 Cookie，也不会把网页内容或会话信息写入诊断日志。可在“设置 → 通知与系统 → Routin 签到”中重新登录或退出登录；退出登录只清除 Routin 网页站点数据，不影响 plan Key、用量缓存或其他设置。若网页结构变化导致应用无法确认结果，请以打开的 Routin 官方页面为准。
