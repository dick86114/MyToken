...
final class GitHubUpdateServiceTests: XCTestCase {
    func test发现较新Release并选择DMG资源() async throws {
        let stub = URLProtocolStub.makeSession { request in
            ...
            return (response, Data(#"{"tag_name":"v1.2.0","html_url":"https://github.com/dick86114/MyRoutin/releases/tag/v1.2.0","body":"修复问题","assets":[{"name":"Routin-Usage.dmg","browser_download_url":"https://example.com/Routin-Usage.dmg"}]}"#.utf8))
        }
        ...
        XCTAssertEqual(update?.downloadURL.absoluteString, "https://example.com/Routin-Usage.dmg")
    }

    func test相同或更旧Release不提示更新() async throws {
        let stub = URLProtocolStub.makeSession { request in
            let response = try XCTUnwrap(HTTPURLResponse(url: try XCTUnwrap(request.url), statusCode: 200, httpVersion: nil, headerFields: nil))
            return (response, Data(#"{"tag_name":"v1.2.0","html_url":"https://github.com/dick86114/MyRoutin/releases/tag/v1.2.0","assets":[{"name":"Routin-Usage.dmg","browser_download_url":"https://example.com/Routin-Usage.dmg"}]}"#.utf8))
        }
